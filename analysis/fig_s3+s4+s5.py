...
for dataset in datasets:
    df_in_scm = pd.read_csv(f'./results/clip_vit_b_32/{dataset}_cossim_scm_t2.csv')
    df_in_abc = pd.read_csv(f'./results/clip_vit_b_32/{dataset}_cossim_abc_t2.csv')
    ...
    if dataset == 'fairface':
        ...
        df_in_scm = pd.read_csv('./results/clip_vit_b_32/causalface_age_cossim_scm_t2.csv').set_index(non_cossim_cols)
        df_in_abc = pd.read_csv('./results/clip_vit_b_32/causalface_age_cossim_abc_t2.csv').set_index(non_cossim_cols)
    ...
    # --------------- differences
    blank = pd.read_csv(f'./results/clip_vit_b_32/{dataset}_cossim_control_t2.csv').set_index("img_path")[
        "cossim_<blank>"]
    ...
    def plot_dimension(ax, x, y, title, df_mean, degree=2, cluster_colors=None):
        ...
        # remove black female entries
        # meanmean = meanmean[~((meanmean.race=="black") & (meanmean.gender=="female"))]
    ...
    f = 1.2
    fig, axes = plt.subplots(2, 4, figsize=(f * 10, f * 5), sharex="all")  # 2 rows, 3 columns
    plot_dimension(axes[0, 0], 'age', 'warmth', 'Warmth', df_mean, cluster_colors={('female', 'black'): '#D81B60'})
    plot_dimension(axes[1, 0], 'age', 'competence', 'Competence', df_mean,
                   cluster_colors={('female', 'black'): '#D81B60'})
    plot_dimension(axes[0, 1], 'age', 'agency_pos', 'Positive Agency', df_mean,
                   cluster_colors={('female', 'black'): '#D81B60'})
    plot_dimension(axes[1, 1], 'age', 'agency_neg', 'Negative Agency', df_mean,
                   cluster_colors={('female', 'black'): '#D81B60'})
    plot_dimension(axes[0, 2], 'age', 'belief_pos', 'Progressive Beliefs', df_mean,
                   cluster_colors={('female', 'black'): '#D81B60'})
    plot_dimension(axes[1, 2], 'age', 'belief_neg', 'Conservative Beliefs', df_mean,
                   cluster_colors={('female', 'black'): '#D81B60'})
    plot_dimension(axes[0, 3], 'age', 'communion_pos', 'Positive Communion', df_mean,
                   cluster_colors={('female', 'black'): '#D81B60'})
    plot_dimension(axes[1, 3], 'age', 'communion_neg', 'Negative Communion', df_mean,
                   cluster_colors={('female', 'black'): '#D81B60'})

    handles, labels = axes[0, 0].get_legend_handles_labels()
    handles = handles[1:3] + handles[4:7]
    labels = labels[1:3] + labels[4:7]
    labels = ["women", "men", "Asian", "Black", "White"]
    axes[1, 0].legend(handles, labels, loc='lower left', fontsize="small")

    axes[0, 0].set_ylabel("$\Delta$ Cosine Similarity (%)")
    axes[1, 0].set_ylabel("$\Delta$ Cosine Similarity (%)")

    plt.tight_layout()
    plt.savefig(f"plots/intersect_age_{dataset}.pdf")
    plt.savefig(f"plots/intersect_age_{dataset}.png", dpi=300)
    plt.savefig(f"plots/intersect_age_{dataset}.svg", format="svg")
